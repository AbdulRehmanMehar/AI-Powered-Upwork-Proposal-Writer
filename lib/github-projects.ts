...
/**
 * Find projects relevant to a job description
 */
export function findRelevantProjects(
  ...
  const techKeywords = extractTechKeywords(jobLower);
  ...
  const scored = projects.map(project => {
    ...
    if (project.language && jobLower.includes(project.language.toLowerCase())) {
      score += 10;
    }

    // Check topic matches
    for (const topic of project.topics) {
      if (jobLower.includes(topic.toLowerCase())) {
        score += 5;
      }
    ...
    if (project.readme) {
      for (const keyword of techKeywords) {
        if (project.readme.toLowerCase().includes(keyword)) {
          score += 1;
        }
      }
    }
    ...
    if (project.stars > 0) score += Math.min(project.stars, 5);

    // Bonus for recent activity
    const monthsAgo = (Date.now() - new Date(project.lastUpdated).getTime()) / (1000 * 60 * 60 * 24 * 30);
...
function extractTechKeywords(text: string): string[] {
  const techTerms = [
    ...
    'c#', 'c++', 'scala', 'elixir', 'clojure', 'haskell',
    // Frontend
    'react', 'vue', 'angular', 'svelte', 'nextjs', 'next.js', 'nuxt', 'gatsby',
    'tailwind', 'css', 'sass', 'html', 'webpack', 'vite',
    // Backend
    'node', 'nodejs', 'express', 'fastify', 'nestjs', 'django', 'flask', 'fastapi',
    'rails', 'spring', 'laravel', 'gin', 'fiber',
    // Databases
    'postgres', 'postgresql', 'mysql', 'mongodb', 'redis', 'elasticsearch',
    'dynamodb', 'supabase', 'firebase', 'prisma', 'drizzle',
    // Cloud/DevOps
    'aws', 'gcp', 'azure', 'docker', 'kubernetes', 'k8s', 'terraform', 'vercel', 'netlify',
    // AI/ML
    'openai', 'langchain', 'llm', 'ai', 'machine learning', 'ml', 'gpt', 'rag',
    'embeddings', 'vector', 'pinecone', 'qdrant', 'chromadb',
    // Other
    'api', 'rest', 'graphql', 'websocket', 'microservices', 'serverless',
    'stripe', 'payments', 'auth', 'oauth', 'jwt', 'saas', 'fintech',
